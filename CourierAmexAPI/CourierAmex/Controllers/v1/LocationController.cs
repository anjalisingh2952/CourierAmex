...
namespace CourierAmex.Controllers
{
    ...
    public class LocationController : ControllerBase
    {
        ...
        [HttpGet("All")]
        public async Task<IActionResult> GetByCompany([FromQuery] int cid = 0, [FromQuery] int sid = 0)
        {
            IEnumerable<LocationModel> response;
            try
            {
                response = await _service.GetByCompanyAsync(cid, sid);
            }
            catch (Exception ex)
            {
                _logger.LogError($"There was an error on 'Location_GetByCompany' invocation. {ex.Message}");
        ...
        {
            GenericResponse<LocationModel>? response;
            try
            {
            ...
            {
                _logger.LogError($"There was an error on 'Location_GetById' invocation. {ex.Message}");
                return StatusCode(400, ex.Message);
            }
        ...
        {
            PagedResponse<LocationModel> response;
            try
            {
            ...
            {
                _logger.LogError($"There was an error on 'Location_GetPaged' invocation. {ex.Message}");
                return StatusCode(400, ex.Message);
            }
        ...
        public async Task<IActionResult> Put([FromBody] LocationModel entity)
        {
            ...
            {
                _logger.LogError($"There was an error on 'Location_Put' invocation. {ex.Message}");
                return StatusCode(400, ex.Message);
            }
        ...
        public async Task<IActionResult> Delete([FromQuery] int id)
        {
            ...
            {
                _logger.LogError($"There was an error on 'Location_Delete' invocation. {ex.Message}");
                return StatusCode(400, ex.Message);
            }

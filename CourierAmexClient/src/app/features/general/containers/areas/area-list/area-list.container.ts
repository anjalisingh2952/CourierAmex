...
@Component({
  selector: 'area-list',
  templateUrl: './area-list.container.html'
})
export class AreaListContainer implements OnInit {
  ...
  selectedZone: ZoneModel | undefined = undefined;
  hasAdd: boolean = false;

  private readonly _entities = new BehaviorSubject<AreaModel[]>([]);
  ...
  private readonly _zones = new BehaviorSubject<ZoneModel[]>([]);
  ...
  constructor(
    ...
    this.hasAdd = this.credentailsService.hasPermission(PermissionsEnum.Areas, PermissionActionEnum.Delete);
  }
  ...
  addNew(): void {
    this.router.navigate(['general', 'area-details']);
  }
  ...
  editEntity(entity: AreaModel): void {
    if (entity.id >= 0) {
      this.router.navigate(['general', 'area-details'], { queryParams: { id: entity.id } });
    }
  }
  ...
  private loadZones(): void {
    ...
    if (stateId >= 0) {
      ...
      this.commonService.getZones$(stateId)
        .pipe(
          filter((res) => res?.length > 0),
          finalize(() => {
            this.loading.hide();
  ...
  private performSearch(): void {
    ...
    const countryId = this.selectedCountry?.id ?? 0;
    const stateId = this.selectedState?.id ?? 0;
    const zoneId = this.selectedZone?.id ?? 0;

    this.areaService.getPaged$(this.pagination, countryId, stateId, zoneId)
      .pipe(

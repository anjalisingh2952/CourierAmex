...
import { CountryModel, StateModel, newStateModel } from '@app/features/general/models';
...
@Component({
  ...
  templateUrl: './state-details.container.html'
})
export class StateDetailsContainer implements OnInit, OnDestroy {
  private readonly _entity = new BehaviorSubject<StateModel>({ ...newStateModel });
  ...
  save(entity: StateModel): void {
    this.loading.show();

    const observer = {
      next: (res: GenericResponse<StateModel>) => {
        if (res?.success && res?.data) {
          Swal.fire(this.messages.getTranslate('Labels.SaveChanges'), this.messages.getTranslate('Labels.SaveSuccessfully'), 'success');
          this.router.navigate(['general', 'state-details'], { queryParams: { id: res.data.id, dt: getDateString() }, replaceUrl: true });
        } else {
    ...
    if (entity.id === 0) {
      this.stateService.create$(entity)
        .pipe(finalize(() => this.loading.hide()))
        .subscribe(observer);
    } else {
      this.stateService.update$(entity)
        .pipe(finalize(() => this.loading.hide()))
        .subscribe(observer);
    }
  }
  ...
  goBack(): void {
    this.router.navigate(['general', 'states']);
  }

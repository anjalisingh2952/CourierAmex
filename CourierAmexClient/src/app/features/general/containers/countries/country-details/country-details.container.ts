...
import { CountryModel, newCountryModel } from '@app/features/general/models';
...
@Component({
  selector: 'settings-country-details',
  templateUrl: './country-details.container.html'
})
export class CountryDetailsContainer implements OnDestroy {
  private readonly _entity = new BehaviorSubject<CountryModel>({ ...newCountryModel });
  ...
  save(entity: CountryModel): void {
    this.loading.show();

    const observer = {
      next: (res: GenericResponse<CountryModel>) => {
        if (res?.success && res?.data) {
          Swal.fire(this.messages.getTranslate('Labels.SaveChanges'), this.messages.getTranslate('Labels.SaveSuccessfully'), 'success');
          this.router.navigate(['general', 'country-details'], { queryParams: { id: res.data.id, dt: getDateString() }, replaceUrl: true });
        } else {

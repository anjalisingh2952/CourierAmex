...
@Component({
  selector: 'app-aeropost-mass-upload',
  templateUrl: './aeropost-mass-upload.component.html',
  styleUrl: './aeropost-mass-upload.component.scss'
})
export class AeropostMassUploadComponent implements OnInit {
  ...
  generateReport(form: NgForm): void {
    ...
    this.loadingService.show();
    const providerId = 378;
    const companyId = this.selectedCompanyId;
    const startDate = this.startDate;
    const endDate = this.endDate;

    this.invoiceService.exportAeropostMassUploadReport(companyId, startDate, endDate, providerId)
      .subscribe({
        next: (blob) => {
          ...
          a.href = objectUrl;
          a.download = 'aeropost_mass_upload.xlsx';

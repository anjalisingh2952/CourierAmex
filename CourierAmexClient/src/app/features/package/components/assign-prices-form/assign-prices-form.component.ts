...
@Component({
  selector: 'app-assign-prices-form',
  templateUrl: './assign-prices-form.component.html',
  inputs: ['entity', 'manifests', 'companies'],
  outputs: ['onCompanyChange', 'onManifestChange', 'onStateChange'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})

export class AssignPricesFormComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  private setDefaultCompany(): void {
      ...
      if (this.credentailsService.isGatewayUser() && this.companies.length > 0) {
        ...
        if (userCia) {
          this.entity.companyId = userCia.id;  

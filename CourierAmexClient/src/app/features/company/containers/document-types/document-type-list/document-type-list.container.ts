...
import { CompanyModel, DocumentTypeModel, DocumentTypeService } from '@app/features/company';

@Component({
  selector: 'document-type-list',
  templateUrl: './document-type-list.container.html'
})
export class DocumentTypeListContainer implements OnInit, AfterViewInit {
  ...
  private readonly _entities = new BehaviorSubject<DocumentTypeModel[]>([]);
  ...
  constructor(
    ...
    this.hasAdd = this.credentailsService.hasPermission(PermissionsEnum.DocumentTypes, PermissionActionEnum.Delete);
    this.hasDelete = this.credentailsService.hasPermission(PermissionsEnum.DocumentTypes, PermissionActionEnum.Delete);
    this.pagination.s = `${this.state.sortColumn} ${this.state.sortDirection}`;
    this.translate.get(['Labels.Name',
      'DocumentTypes.CompanyName',
      'Labels.Status',
      'Labels.Actions'])
      .subscribe(
        translations => {
          this.translations.name = translations['Labels.Name'];
          this.translations.companyName = translations['DocumentTypes.CompanyName'];
  ...
  protected addNew(): void {
    this.router.navigate(['company', 'document-type-details']);
  }

  protected editEntity(entity: DocumentTypeModel): void {
    if (entity.id > 0) {
      this.router.navigate(['company', 'document-type-details'], { queryParams: { id: entity.id } });
    }
  }

  protected deleteEntity(entity: DocumentTypeModel): void {
    Swal.fire({
  ...
  private performSearch(): void {
    ...
    this.documentTypeService.getPaged$(this.pagination, companyId)
      .pipe(

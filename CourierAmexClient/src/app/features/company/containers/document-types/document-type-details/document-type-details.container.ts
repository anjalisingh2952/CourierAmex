...
import { CompanyModel, DocumentTypeModel, DocumentTypeService, newDocumentType } from '@app/features';
...
@Component({
  ...
  templateUrl: './document-type-details.container.html'
})
export class DocumentTypeDetailsContainer implements OnInit, OnDestroy {
  private readonly _entity = new BehaviorSubject<DocumentTypeModel>({ ...newDocumentType });
  ...
  save(entity: DocumentTypeModel): void {
    this.loading.show();

    const observer = {
      next: (res: GenericResponse<DocumentTypeModel>) => {
        if (res?.success && res?.data) {
          Swal.fire(this.messages.getTranslate('Labels.SaveChanges'), this.messages.getTranslate('Labels.SaveSuccessfully'), 'success');
          this.router.navigate(['company', 'document-type-details'], { queryParams: { id: res.data.id, dt: getDateString() }, replaceUrl: true });
        } else {
    ...
    if (entity.id === 0) {
      this.documentTypeService.create$(entity)
        .pipe(finalize(() => this.loading.hide()))
        .subscribe(observer);
    } else {
      this.documentTypeService.update$(entity)
        .pipe(finalize(() => this.loading.hide()))
        .subscribe(observer);
    }
  }
  ...
  goBack(): void {
    this.router.navigate(['company', 'document-types']);
  }
  ...
  private loadEntity(id: number): void {
    if (id > 0) {
      ...
      this.documentTypeService.getById$(id)
        .pipe(

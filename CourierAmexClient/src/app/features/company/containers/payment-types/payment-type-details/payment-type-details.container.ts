...
import { CompanyModel, PaymentTypeModel, PaymentTypeService, newPaymentType } from "@app/features";
...
@Component({
    selector: 'payment-type-details',
    templateUrl: './payment-type-details.container.html'
})
export class PaymentTypeDetailsContainer implements OnInit, OnDestroy {
    private readonly _entity = new BehaviorSubject<PaymentTypeModel>({...newPaymentType});
    ...
    save(entity: PaymentTypeModel): void {
      this.loading.show();
  
      const observer = {
        next: (res: GenericResponse<PaymentTypeModel>) => {
          if (res?.success && res?.data) {
            Swal.fire(this.messages.getTranslate('Labels.SaveChanges'), this.messages.getTranslate('Labels.SaveSuccessfully'), 'success');
            this.router.navigate(['company', 'payment-type-details'], { queryParams: { id: res.data.id, dt: getDateString() }, replaceUrl: true });
          } else {
      ...
      if (entity.id === 0) {
        this.paymentTypeService.create$(entity)
          .pipe(finalize(() => this.loading.hide()))
          .subscribe(observer);
      } else {
        this.paymentTypeService.update$(entity)
          .pipe(finalize(() => this.loading.hide()))
          .subscribe(observer);
      }
    }
    ...
    goBack(): void {
      this.router.navigate(['company', 'payment-types']);
    }
    ...
    private loadEntity(id: number): void {
      if (id > 0) {
        ...
        this.paymentTypeService.getById$(id)
          .pipe(

...
@Component({
  ...
  templateUrl: './location-list.container.html'
})
export class LocationListContainer implements OnInit {
  ...
  private readonly _entities = new BehaviorSubject<LocationModel[]>([]);
  ...
  constructor(
    ...
    this.hasAdd = this.credentailsService.hasPermission(PermissionsEnum.Locations, PermissionActionEnum.Delete);
    this.hasDelete = this.credentailsService.hasPermission(PermissionsEnum.Locations, PermissionActionEnum.Delete);
    ...
    this.translate.get(['Labels.Name',
      ...
      .subscribe(
        translations => {
          ...
          this.translations.companyName = translations['Locations.CompanyName'];
          this.translations.phone = translations['Locations.Phone'];
  ...
  protected addNew(): void {
    this.router.navigate(['company', 'location-details']);
  }

  protected editEntity(entity: LocationModel): void {
    if (entity.id > 0) {
      this.router.navigate(['company', 'location-details'], { queryParams: { id: entity.id } });
    }
  }

  protected deleteEntity(entity: LocationModel): void {
    Swal.fire({
  ...
  private performSearch(): void {
    ...
    this.locationService.getPaged$(this.pagination, companyId)
      .pipe(

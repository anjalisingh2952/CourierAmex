...
import { CompanyModel, CustomerPayTypeModel, CustomerPayTypeService } from '@app/features/company';

@Component({
  selector: 'customer-pay-type-list',
  templateUrl: './customer-pay-type-list.container.html'
})
export class CustomerPayTypeListContainer implements OnInit, AfterViewInit {
  ...
  private readonly _entities = new BehaviorSubject<CustomerPayTypeModel[]>([]);
  ...
  constructor(
    ...
    this.hasAdd = this.credentailsService.hasPermission(PermissionsEnum.CustomerPayTypes, PermissionActionEnum.Add);
    this.hasDelete = this.credentailsService.hasPermission(PermissionsEnum.CustomerPayTypes, PermissionActionEnum.Delete);
    this.pagination.s = `${this.state.sortColumn} ${this.state.sortDirection}`;
    this.translate.get(['Labels.Name',
      'CustomerPayTypes.CompanyName',
      'Labels.Status',
      'Labels.Actions'])
      .subscribe(
        translations => {
          this.translations.name = translations['Labels.Name'];
          this.translations.companyName = translations['CustomerPayTypes.CompanyName'];
  ...
  protected addNew(): void {
    this.router.navigate(['company', 'paytype-details']);
  }

  protected editEntity(entity: CustomerPayTypeModel): void {
    if (entity.id > 0) {
      this.router.navigate(['company', 'paytype-details'], { queryParams: { id: entity.id } });
    }
  }

  protected deleteEntity(entity: CustomerPayTypeModel): void {
    Swal.fire({
  ...
  private confirmDeletion(id: number): void {
    ...
    this.customerPayTypeService.delete$(id)
      .pipe(
        finalize(() => {
          this.loading.hide();
          this.onStateChange({

...
import { ClientCategoryService, CompanyService } from '@app/features/company/services';
import { CredentialsService, LoadingService, MessageService } from '@app/@core';
import { ClientCategoryModel, CompanyModel } from '@app/features/company/models';
import { ColDef, TableState } from '@app/@shared/components/tabular-data/tabular-data.component';
import { TranslateService } from '@ngx-translate/core';

@Component({
  selector: 'company-client-category-list',
  templateUrl: './client-category-list.container.html'
})
export class ClientCategoryListContainer implements OnInit, AfterViewInit {
  ...
  private readonly _entities = new BehaviorSubject<ClientCategoryModel[]>([]);
  ...
  constructor(
    ...
    this.hasAdd = this.credentailsService.hasPermission(PermissionsEnum.ClientCategories, PermissionActionEnum.Delete);
    this.hasDelete = this.credentailsService.hasPermission(PermissionsEnum.ClientCategories, PermissionActionEnum.Delete);
    ...
    this.translate.get(['Labels.Name',
      ...
      .subscribe(
        translations => {
          ...
          this.translations.discount = translations['ClientCategories.Discount'];
  ...
  ngAfterViewInit(): void {
    ...
    this.columns.push({field: 'discount', label:  this.translations.discount, sortable: true});
  ...
  protected addNew(): void {
    this.router.navigate(['company', 'category-details']);
  }

  protected editEntity(entity: ClientCategoryModel): void {
    if (entity.id > 0) {
      this.router.navigate(['company', 'category-details'], { queryParams: { id: entity.id } });
    }
  }

  protected deleteEntity(entity: ClientCategoryModel): void {
    Swal.fire({
  ...
  private confirmDeletion(id: number): void {
    ...
    this.clientCategoryService.delete$(id)
      .pipe(
        finalize(() => {
          this.loading.hide();
          this.onStateChange({
  ...
  private performSearch(): void {
    ...
    this.clientCategoryService.getPaged$(this.pagination, companyId)
      .pipe(

...
import { CompanyModel, newCompanyModel } from '@app/features/company/models';
...
@Component({
  selector: 'company-company-details',
  templateUrl: './company-details.container.html'
})
export class CompanyDetailsContainer implements OnInit, OnDestroy {
  private readonly _entity = new BehaviorSubject<CompanyModel>({ ...newCompanyModel });
  ...
  save(entity: CompanyModel): void {
    this.loading.show();

    const observer = {
      next: (res: GenericResponse<CompanyModel>) => {
        if (res?.success && res?.data) {
          Swal.fire(this.messages.getTranslate('Labels.SaveChanges'), this.messages.getTranslate('Labels.SaveSuccessfully'), 'success');
          this.router.navigate(['company', 'details'], { queryParams: { id: res.data.id, dt: getDateString() }, replaceUrl: true });
        } else {

...
import { CompanyModel, CommodityModel, CommodityService } from '@app/features/company';

@Component({
  selector: 'commodity-list',
  templateUrl: './commodity-list.container.html'
})
export class CommodityListContainer implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.translate.get(['Labels.CompanyName',
      'Labels.Code',
      'Labels.Description',
      'Commodities.CustomsDuty',
      'Commodities.CustomsGct',
      'Commodities.CustomsFee',
      'Labels.Status',
      'Labels.Actions'])
      .subscribe(
        translations => {
          ...
          this.translations.customsDuty = translations['Commodities.CustomsDuty'];
          this.translations.customsGct = translations['Commodities.CustomsGct'];
          this.translations.customsFee = translations['Commodities.CustomsFee'];
  ...
  ngAfterViewInit(): void {
    ...
    this.columns.push({ field: 'customsDuty', label: this.translations.customsDuty, sortable: false, type: '2decimals' });
    this.columns.push({ field: 'customsGct', label: this.translations.customsGct, sortable: false, type: '2decimals' });
    this.columns.push({ field: 'customsFee', label: this.translations.customsFee, sortable: false, type: '2decimals' });
  ...
  protected addNew(): void {
    this.router.navigate(['company', 'commodity-details']);
  }

  protected editEntity(entity: CommodityModel): void {
    if (entity.id > 0) {
      this.router.navigate(['company', 'commodity-details'], { queryParams: { id: entity.id } });
    }
  }

  protected deleteEntity(entity: CommodityModel): void {
    Swal.fire({
  ...
  private performSearch(): void {
    ...
    this.commodityService.getPaged$(this.pagination, companyId)
      .pipe(

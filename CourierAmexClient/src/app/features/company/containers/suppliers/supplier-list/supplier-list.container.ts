...
import { CompanyModel, SupplierModel } from '@app/features/company/models';
import { SupplierService } from '@app/features/company/services';

@Component({
  selector: 'supplier-list',
  templateUrl: './supplier-list.container.html'
})
export class SupplierListContainer implements OnInit, AfterViewInit {
  ...
  private readonly _entities = new BehaviorSubject<SupplierModel[]>([]);
  ...
  constructor(
    ...
    this.hasAdd = this.credentailsService.hasPermission(PermissionsEnum.Suppliers, PermissionActionEnum.Delete);
    this.hasDelete = this.credentailsService.hasPermission(PermissionsEnum.Suppliers, PermissionActionEnum.Delete);
    ...
    this.translate.get(['Labels.Name',
      ...
      .subscribe(
        translations => {
          ...
          this.translations.companyName = translations['Suppliers.CompanyName'];
          this.translations.phone = translations['Suppliers.Phone'];
          this.translations.contact = translations['Suppliers.Contact'];
  ...
  ngAfterViewInit(): void {
    ...
    this.columns.push({ field: 'phone', label: this.translations.phone, sortable: true });
    this.columns.push({ field: 'contact', label: this.translations.contact, sortable: true });
  ...
  protected addNew(): void {
    this.router.navigate(['company', 'supplier-details']);
  }

  protected editEntity(entity: SupplierModel): void {
    if (entity.id > 0) {
      this.router.navigate(['company', 'supplier-details'], { queryParams: { id: entity.id } });
    }
  }

  protected deleteEntity(entity: SupplierModel): void {
    Swal.fire({
  ...
  private performSearch(): void {
    ...
    this.supplierService.getPaged$(this.pagination, companyId)
      .pipe(
